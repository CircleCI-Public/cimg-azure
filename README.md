...
We encourage [issues](https://github.com/CircleCI-Public/cimg-azure/issues) and [pull requests](https://github.com/CircleCI-Public/cimg-azure/pulls) against this repository. In order to value your time, here are some things to consider:

1. We won't include just anything in this image. In order for us to add a tool within the Azure image, it has to be something that is maintained and useful to a large number of CircleCI users. Every tool added makes the image larger and slower for all users so being thorough on what goes in the image will benefit everyone.
